...
const TARGET_BYTES = Number(process.env.TARGET_BYTES || 500000);
...
async function compressImage(buffer, targetBytes) {
  ...
  const sizeRatio = Math.sqrt(targetBytes / buffer.length);
  let scale = Math.min(1, sizeRatio * 1.1);
  let quality = 78;
  ...
  for (let i = 0; i < 8; i += 1) {
    ...
    if (mime === Jimp.MIME_JPEG && quality > 50) {
      quality -= 8;
    } else {
      scale *= 0.85;
    }
...
exports.handler = async (event) => {
  ...
  const targetBytes = Math.min(
    Number(payload && payload.targetBytes) || TARGET_BYTES,
    TARGET_BYTES,
  );
  ...
  if (!contentType || !prefix) {
    return json(400, {
      error: "Unsupported contentType. Use image/* or video/*.",
    });
  }
  ...
  try {
    if (imageBase64) {
      ...
      const { buffer: compressedBuffer, mime } = await compressImage(
        rawBuffer,
        targetBytes,
      );
